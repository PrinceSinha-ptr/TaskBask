package `in`.porter.taskbask.servers.sqs.app

import `in`.porter.kotlinutils.serde.jackson.json.JsonMapper
import `in`.porter.taskbask.api.models.async.AsyncJob
import `in`.porter.taskbask.servers.sqs.configs.customConfigure
import `in`.porter.taskbask.servers.sqs.consumers.AsyncSQSConfig
import `in`.porter.taskbask.servers.sqs.di.SQSComponentFactory
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import org.apache.logging.log4j.kotlin.logger

private val logger = logger("in.porter.taskbask.servers.sqs.app.MainRunnerKt")
